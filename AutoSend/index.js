...
async function sendMail(name, email) {
    try {
        ...
        const mailOptions = {
            ...
            to: email,
            subject: "Tin nhắn xác nhận tự động",
            html: `
                <p>Xin chào ${name},</p>
                ...
                <ul>
                    ...
                    <li>TikTok: <a href="https://www.tiktok.com/@kienduk">kienduk</a> (nếu có chuỗi, hãy nhớ duy trì)</li>
...
async function main() {
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            const { name, email, stt } = data[i];

            if (stt === 0) {  // Kiểm tra trạng thái

                // Cập nhật trạng thái
                const updateResponse = await fetch(`https://script.google.com/macros/s/AKfycbwSZBU9PQomOMKXO5vibtIJejBjXQWfn9zR8HSywjmMlDiwJZUrLqxO2WMEMnWa-HVl/exec?rowIndex=${i + 2}&rowIndex=1`);
                
                if (updateResponse.ok) {
                    const updateData = await updateResponse.text();
                    console.log(updateData); // Log kết quả cập nhật
                    ...
                    console.error("Cập nhật trạng thái không thành công:", updateResponse.statusText);
                }
            }
        }
